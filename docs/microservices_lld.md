...
> **Status:** Phase 1 Active / Phase 2 Specified  
...
10. [DB ownership](#10-db-ownership-phase-1)
...
frontend ──REST──▶ backend ──job.created──▶ RabbitMQ
                   ...
                   └──media BackgroundTask      │
                       ...
                       job.start.*              │
                   ┌───────┴────────┐           │ job.results.*
              stt-service  nmt-service          │
                   │           │                │
                  DB + S3      DB         (advances pipeline)
...
| `output_type` | Phase 1 stages | Phase 2 addition |
|---------------|----------------|------------------|
| `uploadOnly` | (none — complete immediately) | — |
| `captionsOnly` | STT → done | — |
| `captionsAndTranslation` | STT → NMT → done | — |
| `translationAndTTS` | STT → NMT → TTS (Celery bridge) | TTS via RabbitMQ (Phase 2) |
| `fullDubbing` | STT → NMT → TTS (Celery bridge) | + merge stage (Phase 2) |

> Phase 1: `fullDubbing` ends at TTS. The orchestrator comment in `manager.go` line 39 documents this: "Re-add JobTypeDubbingMerge when a dedicated merge worker exists."
...
```
tts-service/
├── Dockerfile  Dockerfile.gpu  requirements.txt  pytest.ini
├── tests/{conftest.py, test_worker.py, test_audio_combine.py}
└── app/
    ├── main.py          # FastAPI + lifespan, port 8003
    ├── config.py        # TTS_INTERNAL_CONCURRENCY, PORT=8003
    ├── worker.py        # consume stage.tts → synth + combine → job.results.tts
    ├── model.py         # SilmaTTSModelManager (from backend/app/tts/models.py, stripped of Celery)
    ├── audio_combine.py # audio half of DubbingMergeService: atempo stretch + concat
    └── storage.py       # S3 up/down for wavs
```

#### Source code extraction

- `model.py` ← `backend/app/tts/models.py`: keep `SilmaTTSModelManager.synthesize()`, remove `register_tts_task`, `synthesize_tts` Celery task
- `audio_combine.py` ← `backend/app/dubbing/service.py`: `_apply_time_stretch`, `_fit_audio_to_duration`, `_merge_audio_segments`, `_generate_silence` — the AUDIO-only half of `DubbingMergeService`

#### What it does

1. Load translated segments from `video_tasks.segments`
2. Synthesize each segment in a bounded `ThreadPoolExecutor` (D4 — internal fan-out, no Redis counter)
...
```sql
UPDATE video_tasks SET combined_audio_key=:key, updated_at=:now WHERE id=:task_id;
UPDATE jobs SET status='COMPLETED', progress=75.0, ... WHERE id=:job_id;
```
...
### `fullDubbing` (Phase 1: STT+NMT validated; TTS still Celery bridge)
...
| Health Check API | Every service: /health, /readiness, /health/model |
...
| Test file | What it covers |
|-----------|---------------|
| `backend/tests/integration/test_orchestrator.py` | Full state machine with fake workers; drives `fullDubbing` through STT→NMT→TTS |
| `backend/tests/integration/test_captions_only.py` | `captionsOnly`: publish `job.created` → wait for `job.start.stt` → respond COMPLETED → assert parent COMPLETED |
| `backend/tests/integration/test_captions_and_translation.py` | `captionsAndTranslation`: STT + NMT → assert parent COMPLETED |
...
- [ ] `docs/microservices_lld.md` exists (this file) with full-app design incl. Phase 2 TTS/merge specs
- [ ] `libs/dablja-worker` installed by stt-service and nmt-service; local `app/dablja_worker.py` copies removed
- [ ] `stt-service/tests/test_worker.py` passes with `pytest` (unit, no infra)
- [ ] `nmt-service/tests/test_worker_logic.py` covers all output_types + segment shape contract
- [ ] `docker-compose.test.yml` exists and `run_integration_tests.sh` succeeds
- [ ] `helpers.py` uses port 5672 (matching docker-compose.yml)
- [ ] `captionsOnly` E2E script runs successfully against live stack
- [ ] `captionsAndTranslation` E2E script runs successfully against live stack
- [ ] Go unit tests pass: `go test ./internal/pipeline/...`
- [ ] Backend CI (`backend-tests.yml`) + Go CI (`go-tests.yml`) workflows enabled
- [ ] Celery still handles TTS paths — unchanged, documented as Phase 2
