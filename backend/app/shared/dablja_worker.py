"""Shared RabbitMQ consumer utilities for pipeline workers (H2/M1/H3/DG)."""
import logging
import time
...
def make_engine(database_url: str) -> tuple:
    """Return a cached (engine, SessionLocal) pair for the given database URL (M1)."""
...
def classify_failure(exc: BaseException) -> Literal["transient", "permanent"]:
    """Classify an exception for nack-requeue vs ack-and-fail handling (H3/DG)."""
    if isinstance(
        exc,
        (
            AMQPConnectionError,
...
def check_cancelled(db, job_id: str) -> bool:
    """Return True when the job row exists and status is CANCELLED (DG)."""
    row = db.execute(
...
def consume_loop(
    ...
    """Blocking consume loop with exponential backoff reconnect on connection loss (H2)."""
    ...
    while True:
        try:
            ...
            logger.warning("[%s] RabbitMQ connection lost: %s — reconnecting in %.1fs", service_name, exc, backoff)
